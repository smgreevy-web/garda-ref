...
function doSearch(q,filter,keep){
  ...
  for(const h of hits.slice(0,40)){
    const s=Math.max(0,h.first-60), snip=h.txt.slice(s,h.first+140).replace(/\s+/g,' ');
    let marked=esc(snip);
...
function renderCases(){
  ...
  const draw=()=>{
    ...
    list.innerHTML=html||'<div class="empty">No case matches.</div>';
    ...
    list.querySelectorAll('.cshead').forEach(h=>h.addEventListener('click',()=>h.nextElementSibling.classList.toggle('hidden')));
...
async function renderJudgments(court){
  ...
  try{
    if(!window._bailii){
      const r=await fetch('https://api.allorigins.win/raw?url='+encodeURIComponent('https://www.bailii.org/recent-accessions-ie.html'));
      if(!r.ok)throw new Error('feed unavailable');
      window._bailii=await r.text();
    }
...
function openPage(abs){
  ...
  rdBody.innerHTML=formatPage(txt);
