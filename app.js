...
let META=null, AZ=null, CASES=null, OPS=null, OPS2=null, TPL=null, STEN=null, KB=null;
...
(async function boot(){
  ...
  const [m,a,cs,op,sn,kb,o2,tpl]=await Promise.all(['meta','az','cases','ops','stencils','kb','ops2','templates'].map(f=>fetch('data/'+f+'.json').then(r=>r.json())));
  META=m; AZ=a; CASES=cs; OPS=op; OPS2=o2; TPL=tpl; STEN=sn; KB=kb;
...
function renderTools(){
  view.innerHTML=`<h2 class="sec">Tools</h2><div class="toolmenu">
    ...
    <button class="tmenu" data-v="exh"><b>📦 Exhibit log</b><small>SMG1–SMG99 · voice entry · continuity</small></button>
...
function renderExhibits(){
  const KEY='gr_exhibits';
  let items=JSON.parse(localStorage.getItem(KEY)||'[]');
  const save=()=>localStorage.setItem(KEY,JSON.stringify(items));
  const draw=()=>{
    let rows=items.map((it,i)=>`<div class="tool"><h3>SMG${i+1}</h3>
      <textarea class="fin" data-i="${i}" rows="2" placeholder="Describe exhibit SMG${i+1}…">${esc(it.desc||'')}</textarea>
      <input class="fin" data-c="${i}" placeholder="Continuity: found by / sealed / handed to…" value="${esc(it.cont||'')}">
      <button class="csall mic" data-m="${i}">🎤 Dictate description</button>
      <button class="toolreset" data-del="${i}">Delete</button></div>`).join('');
    const full=items.map((it,i)=>`SMG${i+1}: ${it.desc||''}${it.cont?'  [Continuity: '+it.cont+']':''}`).join('\n');
    $('#exList').innerHTML=rows+(items.length?`<div class="tool"><h3>For your covering report</h3><div style="font-size:13px;white-space:pre-wrap">${esc(full)}</div>${copyBtn(full,'Copy all exhibits')}</div>`:'<div class="empty">No exhibits yet — add SMG1.</div>');
    $$('#exList textarea[data-i]').forEach(t=>t.addEventListener('input',e=>{items[+e.target.dataset.i].desc=e.target.value;save();}));
    $$('#exList input[data-c]').forEach(t=>t.addEventListener('input',e=>{items[+e.target.dataset.c].cont=e.target.value;save();}));
    $$('#exList [data-del]').forEach(b=>b.addEventListener('click',()=>{items.splice(+b.dataset.del,1);save();draw();}));
    $$('#exList .mic').forEach(b=>b.addEventListener('click',()=>dictate(items[+b.dataset.m],draw,save)));
  };
  view.innerHTML=kbHead('📦 Exhibits builder')+`<div class="tool"><button class="csall" id="exAdd">+ Add exhibit (SMG${items.length+1})</button>
    <div class="toolnote" style="margin-top:6px">Stored on THIS phone only. Never enter anything that shouldn't sit on a personal device — this is a drafting aid, not an evidence store.</div></div><div id="exList"></div>`;
  wireBack();
  $('#exAdd').addEventListener('click',()=>{items.push({desc:'',cont:''});save();draw();$('#exAdd').textContent='+ Add exhibit (SMG'+(items.length+1)+')';});
  draw();view.scrollTop=0;
}
...
let EXH=JSON.parse(sessionStorage.getItem('gr_exh')||'[]');
let exhPrefix=localStorage.getItem('gr_exhpre')||'SMG';
function saveExh(){sessionStorage.setItem('gr_exh',JSON.stringify(EXH));}
function renderExhibits(){
